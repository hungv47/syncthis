# Agent setup for `syncthis`

This is a deepsec scanning workspace. Project `syncthis` was just registered
(target: `..`). Setup is incomplete — `data/syncthis/INFO.md`
...
2. **Fill in `data/syncthis/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id syncthis
pnpm deepsec process --project-id syncthis
```
