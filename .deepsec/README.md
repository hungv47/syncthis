...
Currently configured project: `syncthis` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/syncthis/SETUP.md` to fill in
   `data/syncthis/INFO.md`.
...
`--project-id syncthis` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/syncthis/`.
